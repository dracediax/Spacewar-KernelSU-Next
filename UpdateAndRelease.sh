...
[ -f "temp_susfs4ksu/kernel_patches/include/linux/mac_user_macros.h" ] && \
...
# Apply 5.4 compat to susfs.c (fsnotify_ops.handle_inode_event → .handle_event)
python3 -c '
...
if legacy_ops in text:
    ...
    with open(file_path, "w") as f:
        f.write(text)
    print("   ✅ SUSFS 5.4 compat applied.")
else:
    print("   ℹ️  SUSFS compat already applied or pattern changed.")
'
